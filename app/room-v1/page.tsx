...
const Clubhouse: React.FC = () => {
    ...
    const [isCreateRoom, setIsCreateRoom] = useState(target === 'create-room')
    ...
    useEffect(() => {
        ...
        setIsCreateRoom(searchParams?.get('target') === 'create-room')
    }, [searchParams])
                ...
                {!session.isConnected && (
                    <div className="flex flex-col items-center justify-center h-full max-w-87.5 mx-auto gap-6 px-4">
                        ...
                        <div className="w-full space-y-4 border p-6 rounded-lg bg-card">
                            {isCreateRoom && <h3 className='text-lg'>Create a New Room</h3>}
                            {error && <p className="text-destructive text-sm text-center">{error}</p>}
                            <div className="space-y-2">
                                <Label htmlFor="room">Room Name</Label>
                                <Input id="room" value={roomName} disabled={!isCreateRoom} onChange={(e) => setRoomName(e.target.value)} />
                            </div>
                            <div className="space-y-2">
